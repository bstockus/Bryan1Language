...
namespace Bryan1Language.Parser {
	...
	public class BasicParser : IParser {
		...
		private TypeNode ParseType() {
			...
			NodeRange nr = new NodeRange(t.Range.Begin, t.Range.End);

			if (BasicParser.IsIntKeyword(t)) {
				this.Consume();
				return new TypeNode(nr, TypeNode.Type.Integer);
			} else if (BasicParser.IsFloatKeyword(t)) {
				this.Consume();
				return new TypeNode(nr, TypeNode.Type.Float);
			} else if (BasicParser.IsStrKeyword(t)) {
				this.Consume();
				return new TypeNode(nr, TypeNode.Type.String);
			} else if (BasicParser.IsBoolKeyword(t)) {
				this.Consume();
				return new TypeNode(nr, TypeNode.Type.Boolean);
			} else {
		...
		private IdentifierNode ParseIdentifier() {
			...
			if (BasicParser.IsIdentifier(t)) {
				this.Consume();
				return new IdentifierNode(nr, ((IdentifierToken)t).Value);
			} else {
		...
		private StringLiteralNode ParseStringLiteral() {
			...
			if (BasicParser.IsStringLiteral(t)) {
				this.Consume();
				return new StringLiteralNode(nr, ((LiteralToken<string>)t).Value);
			} else {
		...
		private IntegerLiteralNode ParseIntegerLiteral() {
			...
			if (BasicParser.IsIntegerLiteral(t)) {
				this.Consume();
				return new IntegerLiteralNode(nr, ((LiteralToken<int>)t).Value);
			} else {
		...
		private FloatLiteralNode ParseFloatLiteral() {
			...
			if (BasicParser.IsFloatLiteral(t)) {
				this.Consume();
				return new FloatLiteralNode(nr, ((LiteralToken<double>)t).Value);
			} else {
		...
		private BooleanLiteralNode ParseBooleanLiteral() {
			...
			if (BasicParser.IsTrueKeyword(t)) {
				this.Consume();
				return new BooleanLiteralNode(nr, true);
			} else if (BasicParser.IsFalseKeyword(t)) {
				this.Consume();
				return new BooleanLiteralNode(nr, false);
			} else {
		...
		private LetStatementNode ParseLetStatement() {
			...
			return new LetStatementNode(null, expression, null);
